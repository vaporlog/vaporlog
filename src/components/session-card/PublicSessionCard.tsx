...
export default function PublicSessionCard({
  ...
  return (
    <article
      aria-label={`${strainName} session by ${session.author}`}
    ...
    >
      ...
      <p className="mt-3 text-sm text-muted-foreground">
        by <span className="font-medium text-foreground">{session.author}</span>
        {dateLabel !== null && ` · ${dateLabel}`}
      </p>

      {/* Rating — the one accent highlight */}
      <div className="mt-8" aria-label={`Rated ${session.rating} out of 10`}>
